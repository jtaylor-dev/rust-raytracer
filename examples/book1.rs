...
fn main() {
    let matches = App::new("Raytracer")
        ...
        .arg(
            Arg::with_name("depth")
                ...
                .takes_value(true)
                .default_value("50")
                .help("Sets max depth (bounces) for each raycast"),
        )
        .arg(
    ...
    let max_depth: u64 = matches.value_of("depth").unwrap().parse().unwrap();
    let fov: f64 = matches.value_of("fov").unwrap().parse().unwrap();
    let aperture: f64 = matches.value_of("aperture").unwrap().parse().unwrap();
    let focus_distance: f64 = matches.value_of("focus").unwrap().parse().unwrap();
    ...
    for a in -11..11 {
        for b in -11..11 {
            ...
            if (center - Point3::new(4.0, 0.2, 0.0)).length() > 0.9 {
                ...
                scene.add(SpherePrimitive::new(Sphere::new(center, 0.2), &material));
            }
        }
    }
